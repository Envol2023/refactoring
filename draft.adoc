...
L'agilité est une démarche qui prend pour partie de ne pas prétendre dès le départ à être exhaustif, mais d'apporter des incréments de fonctionnalités et de s'assurer que le client est satisfait.
...
[NOTE.notes]
--
La question de la confiance accordée aux personnes en charge du développement est primordiale.

Le manifeste dit _"The build comes around actuated people. Offer them the setting and support they have, and trust them to urge the task done."_

Ce que l'on peut traduire par _"La construction s'appuie sur des personnes motivées. Offrez-leur le cadre et le soutien dont ils disposent et faites-leur confiance pour mener à bien leur tâche."_

Du point de vue managérial, nous passons d'une culture de la surveillance de cette espèce d'ouvriers spécialisés que sont les développeurs à des acteurs digne de confiance qui sauront trouver des solutions.

**Mais** ce point de vue va avec l'excellence. C'est à dire que en même temps, le manager doit s'assurer que l'excellence est possible, que les membres de l'équipes sont motivés, que leurs compétences peuvent s'épanouir et s'acquérir. Cela peut donc présupposer un accompagnement, notamment des débutants ou des personnes dont ce n'est pas le coeur de métier.
--
...
=== Dette technique et culpabilité

**ATTENTION** la dette technique est une métrique de qualité, pas un sujet de honte qui doit être caché et mal vécu: nous produisons de la dette technique par qu'il est extrêmement difficile d'être juste et pertinent tout le temps: l'idée c'est qu'il faut la gérer et la résorber avant qu'elle ne soit trop importante.
...
** Focus sur l'agencement du code
** Pas sur ce qu'il produit ou comment son exécution se comporte
...
[NOTE.notes]
--
La grille de lecture du refactoring ne porte pas sur l'exécution d'un programme, car on le suppose suffisamment satisfaisant, ne serait-ce que le temps de procéder à la modification de son code. Le refactoring se concentre la notion de modification du code et ses effets comme objets principaux. Il pourrait être considéré, dans sa forme la plus extrème, comme un jeu intellectuel, aux effets très concrets pourtant dans la vie d'un projet.
--
...
"Le veritable test pour savoir si un code est bon est de mesurer sa facilité à le modifier"

* Le refactoring se concentre sur l'évolution du code 
* Il est à l'opposé de la conception du "Tombe-en-marche"
* Le refactoring veut minimiser les efforts des humains, 
...
=== Faciliter la compréhension [à mettre dans la partie compréhension]

* Compréhension
** Pour les futurs personnes qui reprendront le code
** Pour comprendre soi-même le code
* Voir l'état du code tel qu'il est **réellement**
** Refactoriser permet de voir les abstractions cachées, les mécanismes utilisées
** "C'est comme nettoyer une fenêtre pour voir ce qu'il y a réellement dans le code"

=== Améliorer la conception [à mettre dans la partie conception]

* Entropie de la conception interne
** Perte du schéma originel, multiples intervenants, patches, mauvaises compréhensions 
* Cercle vicieux à casser
** Vision interne difficile à garder en tête
** Lisibilité mauvaise
** dégradation de la cohérence


=== Amliorer la confiance [à mettre dans la partie testabilité]

* La confiance dans un code repose sur les tests, et/ou des processus de qualité (revues de code)
* Cercle vicieux
** Mauvaise conception => Difficulté à mettre sous tests
** Peu de tests => peu de confiance
*** Frankenstein
** Peu de confiance => Difficulter à le faire évoluer sans risque
** Pas d'évolution => pas d'amélioration de la confiance

=== Être plus productif [à mettre dans la prtie conclusion]

* Dégradation de la conception interne => dégradation productivité
** code plus délicat à tester
** code plus dur à faire évoluer, à débugger
** attractivitée décroissante (effet nouveau projet/side project)
* Faible compréhension => Augmentation du temps consacré à l'ajout d'une fonctionnalité pour recomprendre tous les impacts

Refactoring =/= coût
Refactoring => investissement
...
=== Refactoring: Activité particulière

Non, le refactoring n'est pas une phase particulière
Elle fait partie des outils qu'un développeur(se) professionnel(le) doit utiliser
...
=== Refactoring et juste nécessaire

Le refactoring permet de se libérer de la contrainte de la conception parfaite, idéale, sans retour et exhaustive.

Il permet, techniquement, de suivre le crédo de l'agilité, de n'apporter que l'incrément attendu par le client, juste nécessaire.

Toutefois, le refactoring va avec une bonne culture développement et une capacité de conception. La vision itérative n'est réalisable qu'avec des connception raisonnablement flexibles. Elle nécessite des tests.
Elle nécecsite une bonne culture dev (bonnes pratiques, patterns et antri-patterns) pour ne pas se fourvoyer dans un cul de sac.

=== Refactoring et performances

* Optimization is evil <= Optimization by design nécessaire que dans des cas très specifiques.
* Les mécanismes d'optimisation ne sont pas toujours compris des développeurs
** Optimisation par les compilateurs
** Inlignages
** JVM
* Sans profilage, il est vain de penser optimisation car les problèmes ne viennent pas forcément de là où l'on pense
* Une bonne conception permet de mieux identifier les goulets d'étranglement
...
Les techniques de refactoring sont des 

* **recettes**
* Pensées pour être mises en oeuvre avec des outils primitifs
** Editeur de texte simple
** Framework de tests
* Approches de petits pas
** Chaque pas laisse le code fonctionnel
** => A tout moment il peut être interrompu, même dans un état pas très propre. 
* Cycle
** modification
** test
** commit
...
=== Illustration d'une technique simple :

Renommage fonction **SANS OUTIL de TESTS**
...
Introduire les doctest

=== Comment savoir qu'un test fonctionne ?

Première solution : AYEZ CONFIANCE 
Vous savez que votre code est presque parfait Vous espérez qu'il n'y a pas de bogue
Mais la foi est une affaire privée...

Deuxième solution : TESTER A LA MAIN
Exécuter quelques cas d'utilisation, avec quelques paramètres et vérifier le résultat,Cela fonctionne assez bien... pour des programmes très courts,Ce n'est pas humainement viable lorsque la base de code grandit.
 
Troisième solution : TESTS AUTOMATIQUES 
Utiliser des tests exécutables, exécuter des tests à volonté,Nécessite un investissement mais très rentable.

=== Philosophie d'un test

Le test consiste à s'assurer du comportement 
d'un objet testé (Objet sous test), dans un contexte donné, en vérifiant certaines assertions. (Assertions)
...
GIVEN a guitar that I use by strumming the strings
WHEN I put my hand on the strings 
THEN I can AFFIRM that the sound is muted.
...
Test concernant une unité de programmation : une classe, une fonction, ... 

Les tests unitaires sont des tests techniques créés et utilisés par les développeurs. 
* l'intérieur du composant
* les méthodes et les fonctions de l'ensemble de la classe

=== Test d'intégration 

Test portant sur plusieurs unités de programmation intégrées qui interagissent ensemble.

Le test d'intégration est un test technique, créé et utilisé par les développeurs. 
Le champ d'application du test d'intégration se concentre sur 
* l'extérieur des composants
* l'interaction entre les composants l'interface du composant
les composants en tant que partie d'un système.

=== Test d'acceptation

Test d'un point de vue externe, utilisant une séquence significative d'actions.

Le test d'acceptation est un test fonctionnel, créé et utilisé par les utilisateurs. 

* le comportement de l'application d'un point de vue externe, 
* sans connaissance du contenu technique.

=== Focus sur le test unitaire

* Rapide: 
** Moins de 1 seconde
* Indépendant
** Les tests unitaires peuvent être exécutés dans n'importe quel ordre.
** Aucune dépendance à l'égard de l'état des tests précédents, aucune influence sur les tests suivants.
** Vous pouvez exécuter un test seulement ou tous les tests, cela ne devrait pas avoir d'importance.
* Répétables
** Les tests unitaires ne doivent pas être liés à l'environnement 
***créseau, base de données, système de fichiers, paramètres utilisateur, données sur disque...
** Les tests unitaires doivent donner les mêmes résultats indépendamment
de l'ordinateur, de l'utilisateur, du temps
Que tester ?

=== Quoi tester

Ne pas tout tester
Tester les fonctionnalités importantes et/ou fondamentales
Tester les conceptions complexes
Tester les valeurs limites.
Tester les exceptions et les erreurs.
Tester les arguments illégaux ou les mauvaises entrées.
...
**Recopoier image dans cours java ou Oop**
...
** Permet de communiquer et contractualiser entre groupes de personnes
...
Cf. Chapitre 16 de legacy 

* Raconter verbalement 
* Jouer avec des cartes CRC
** qu'est-ce que je dois savoir ? 
** avec qui j'interagis 
** de quoi n aïs pas besoin
* Écouter comment on parle du code
...
=== Une classe trop grosse

Symptômes

* Trop de propriétés (variables d'instance)
* Du mal à définir ce qu'elle fait vraiment

Conseils

* Extraire des classes, 
* Extraire des sous-classes, 
* Extraire des interfaces,
* En cas de gestion de collections avec des informations complémentaires, assembler la collection avec les propriétés supplémentaires en une seule et même classe. (Replace Data value with object)

=== Une trop longue liste de paramètres 
...
Pureté design vs temps et efforts à consacrer. 

Une fois testée, on peut rendre plus propre 
